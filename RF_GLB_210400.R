...
setwd("C:/Users/WSALLS/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/LandscapeAnalysis/Publication") # REMOVE IN FINAL VERSION ******XXXXXXXXXX*******
...
if (isTRUE(run_cors)) {
  ...
  (sum(cor_preds > 0.7) - ncol(cor_preds)) / 2 # how many correlations exceed 0.7?
  ...
  (sum(cor_preds < 0.3)) / 2 # how many correlations are below 0.3?
...
nruns <- 100 # number of RF runs (not to be confused with ntree)
...
for (j in 1:nruns) {
  ...
  rf_eval_df <- rbind(rf_eval_df, data.frame(run = j,
                                             n = nrow(pred_data),
                                             Mean_CI = mean(resp_data),
                                             Median_CI = median(resp_data),
                                             Min_CI = min(resp_data),
                                             Max_CI = max(resp_data),
                                             SD_CI = sd(resp_data),
                                             MSE_OOB_mean = mean(rf_i$mse),
                                             RMSE_OOB_mean = mean(sqrt(rf_i$mse)),
                                             Rsq_OOB_mean = mean(rf_i$rsq)))
...
write.csv(tree_eval_df, sprintf("out/tree_eval_%s_%s_%s.csv", resp_var, lakes_region, Sys.Date()))
write.csv(rf_eval_df, sprintf("out/rf_eval_%s_%s_%s.csv", resp_var, lakes_region, Sys.Date()))
write.csv(var_imp_df, sprintf("out/var_imp_%s_%s_%s.csv", resp_var, lakes_region, Sys.Date()))
...
for (f in 1:length(eval_files)) {
  ...
  colmeans <- as.data.frame(t(apply(eval_csv[, c("n", "MSE_OOB_mean", "RMSE_OOB_mean", "Rsq_OOB_mean", "Mean_CI", "Median_CI", "Min_CI", "Max_CI", "SD_CI")], 
                                    2, mean)))
